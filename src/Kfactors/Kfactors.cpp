...
double K_integrand(const std::vector<double>& p, double masssquared, int switchvalue,int s, double Temp){
  ...
  switch (switchvalue) {
    ...
    case 6: res = (E0*E0-pz*pz)/(2.0*std::pow(E0,3)) * distribution_f0(E0,s,Temp,1);break;
    case 7: res = std::abs(pz)/(2*E0*E0*E0z) *( distribution_f0(E0,s,Temp,1)/E0 - distribution_f0(E0,s,Temp,2));break;
    case 8: res = std::abs(pz)*distribution_f0(E0,s,Temp,1)/(2.0*E0*E0*E0z);break;
    case 9: res = std::abs(pz)/(4*std::pow(E0,3)*E0z) * (distribution_f0(E0,s,Temp,1)/E0 - distribution_f0(E0,s,Temp,2));break;
    case 10: res = std::abs(pz)*distribution_f0(E0,s,Temp,0)/(2.0*std::pow(E0,3)*E0z);break;
...
double K_integration(double masssquared,double Temp, int switchvalue, int s){
  ...
  double xl[dim] = {0,0,0}; // We integrate from 0 to + infinity and multiply by 2^3 using the symmetry p -> -p
...
double Ktilde_normalization(double Temp, int s, double masssquared){
  ...
  double xmax  = 30; //around x = 30 the integrand becomes < 10^-10 for m^2 = 0. For m^2 > 0 this happens earlierer.
  gsl_integration_qags(&F,0,xmax,0,1e-9,workspace_size,w,&result,&error);

...
double CalculateNorm2(const double &msquared, const double &T, const int &s)
{
  ...
  static const boost_cubic_b_spline<double> KtildeNormalisationFermionSpline(
      Data::KtildeNormFermion_grid.data(),
      Data::KtildeNormFermion_grid.size(),
      0,
      1);

  static const boost_cubic_b_spline<double> KtildeNormalisationBosonSpline(
      Data::KtildeNormBoson_grid.data(),
...
std::unique_ptr<gsl_spline2d, decltype(&gsl_spline2d_free)>
initializeK1fermionGrid()
{
  auto grid = std::unique_ptr<gsl_spline2d, decltype(&gsl_spline2d_free)>(
      gsl_spline2d_alloc(gsl_interp2d_bicubic, Data::msg_size, Data::Tg_size),
      gsl_spline2d_free);
...
{
  static const std::unique_ptr<gsl_spline2d, decltype(&gsl_spline2d_free)>
      grid = initializeK1fermionGrid();
  ...
  double res =
      gsl_spline2d_eval(grid.get(), msquared, T, xacc.get(), yacc.get());
  return res;
}

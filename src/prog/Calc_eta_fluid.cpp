...
try
{
  ...
  if (!(argc == 7 or argc == 8))
  {
    std::cerr << "./Calc_eta_fluid Model Inputfile Outputfile  LineStart "
                 "LineEnd v_W  \n";
    ShowInputError();
    return EXIT_FAILURE;
  }
  ...
  // Initialise the polarisation for the KFACTORs ---> Not needed in the fluid
  // type ansatz
  ...
  */
  ...
  while (getline(infile, linestr))
  {
    ...
    if (linecounter >= LineStart and linecounter <= LineEnd and
        linecounter != 1)
    {
      if (TerminalOutput)
      {
      ...
      if (debug)
        std::cout << "End of parameter inputread -> PTFinder_gen_all called"
                  << std::endl;
      ...
      if (sol.at(2) == 1 and C_PT * sol.at(0) < sol.at(1))
      {
        ...
        for (std::size_t i = 3; i < ndim + 3; i++)
          vcritical.push_back(sol.at(i));
        int CPoddIndex = 3, CPevenIndex = 2;
        double brokenPhase =
            std::atan2(vcritical.at(CPoddIndex), vcritical.at(CPevenIndex));
        // Sign Check for the broken phase
        if (std::abs(brokenPhase) > 0.5 * M_PI)
        {
          for (auto symmetry : modelPointer->SignSymmetries)
          {
            double tmpphase = std::atan2(
                symmetry.at(CPoddIndex) * vcritical.at(CPoddIndex),
                symmetry.at(CPevenIndex) * vcritical.at(CPevenIndex));
            if (std::abs(tmpphase) <= 0.5 * M_PI)
            {
              for (std::size_t i = 0; i < vcritical.size(); i++)
                vcritical.at(i) *= symmetry.at(i);
              break;
            }
          }
        }
        ...
        for (std::size_t i = 0; i < modelPointer->nVEV; i++)
          startpoint.push_back(0.5 * vcritical.at(i));
        Minimize_gen_all(modelPointer,
                         TC + 1,
                         vevsymmetricSolutiontmp,
                         checksym,
                         startpoint);
        ...
        if (debug)
        {
          ...
          {
            std::cout << "vevsymmetricSolution[" << i
                      << "] = " << vevsymmetricSolutiontmp.at(i) << std::endl;
          }
        }
        double absVEVSymmetricSolutionTmp = 0;
        for (auto x : vevsymmetricSolutiontmp)
          absVEVSymmetricSolutionTmp += std::pow(x, 2);
        if (absVEVSymmetricSolutionTmp != 0)
          std::cout << "Check for sign of non vanishing components of "
                       "symmetric vacuum : "
                    << std::endl;
        ...
        // init needs vevsymmetricSolutiontmp! The real vevconfig for the actual
        // top phase is calculated in init() --> vev_sym_theta
        GSL_integration_mubl_container.init(vw,
                                            vcritical,
                                            vevsymmetricSolutiontmp,
                                            TC,
                                            modelPointer,
        ...
        if (debug)
          std::cout << "zmax = " << GSL_integration_mubl_container.getZMAX()
                    << std::endl;
        ...
        // Initialisation of fluid transport class for the numerical evaluation
        ...
        case 1:
          arr_nL =
              set_up_nL_grid(n_step, GSL_integration_mubl_container, C_top);
          break;
        case 2:
          arr_nL =
              set_up_nL_grid(n_step, GSL_integration_mubl_container, C_bot);
          break;
        case 3:
          arr_nL =
              set_up_nL_grid(n_step, GSL_integration_mubl_container, C_tau);
          break;
        default:
          throw std::runtime_error("No valid transport method chosen \n");
        }

        // 	//Initialisation of the eta class for the numercial evaluation of
        // eta
        Calc_eta C_eta;

...
struct CLIOptions_NLOVEV
{
  ...
  CLIOptions_NLOVEV(const BSMPT::parser &argparser);
  bool good() const;
};
...
try
{
  ...
  const CLIOptions_NLOVEV args(argparser);
  if (not args.good())
  {
    return EXIT_FAILURE;
  }
...
bool CLIOptions_NLOVEV::good() const
{
...
CLIOptions_NLOVEV::CLIOptions_NLOVEV(const BSMPT::parser &argparser)
{

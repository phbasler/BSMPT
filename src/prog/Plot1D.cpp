...
/**
 * @file
 * This program calculates the Minimum for a given parameterpoint at a given
 * Temperature and plots the potential as a function V(t*v,Temp) where v is the
 * vector showing to the minimum coming from 0. t is in the range of
 * [FacStart,FacEnd] with FacStep number of Steps.
 */
...
int main(int argc, char *argv[])
{
  ...
  if (!(argc == 9))
  {
    std::cout << "./Plot1D InputType Infile Outfile LinNum Temp FacStart "
                 "FacEnd FacStep \n";
    return -1;
  }
  char *in_file;
  ...
  for (int k = 0; k < dim; k++)
    vTree.push_back(modelPointer->vevTreeMin.at(k));
  ...
  if (vev == 0)
  {
    ...
    for (int i = 0; i < dim; i++)
      sol[i] = modelPointer->vevTreeMin.at(i);
  }

  for (int i = 0; i < dim; i++)
    outfile << sol.at(i) << sep;
  outfile << std::endl;
  for (int nStep = 0; nStep <= FacStep; nStep++)
  {
    ...
    pointPot.clear();
    for (int i = 0; i < dim; i++)
      point.push_back(tStep * sol.at(i));

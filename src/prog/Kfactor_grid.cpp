...
void CreateKfunctionsgrid(){
    ...
    std::vector<std::vector<double>> K1p,K2p,K4p,K5p,K6p,K8p,K9p,K1m,K4m,K5m;

    std::vector<double> masslist, Tlist;
    for(double im = 0;im <= 5; im+= 1e-3) masslist.push_back(im);
    for(double im = 10;im <= std::pow(200,2); im+= 5) masslist.push_back(im);
    for(double it=10;it<=250;it+=2) Tlist.push_back(it);

    for(auto im: masslist){
        std::vector<double> K1p_row, K2p_row, K4p_row, K5p_row, K6p_row, K8p_row, K9p_row, K1m_row, K4m_row, K5m_row;
        for( auto it: Tlist){
    ...
    header << "#ifndef SRC_BARYO_CALCULATION_KFACTORS_GRID_KFUNCTIONS_GRID_H_NEW" << std::endl
        << "#define SRC_BARYO_CALCULATION_KFACTORS_GRID_KFUNCTIONS_GRID_H_NEW" << std::endl;

...
/**
 * @file
 * Calculates eta as a function of the renormalised scale mu. The
...
try
{
  ...
  while (getline(infile, linestr))
  {
    ...
    if (linecounter == 1)
    {
      ...
      for (const auto &x : etaLegend)
        outfile << sep << x + "_muvar";
      outfile << std::endl;
    ...
    if (args.Line == linecounter)
    {
      ...
      for (int step = 0; step <= args.NumberOfSteps; step++)
      {
        ...
        if (args.TerminalOutput)
          std::cout << "\r currently mu_factor = " << mu_factor << std::endl;
        ...
        for (const auto &x : EWPT_mu.EWMinimum)
          startpoint.push_back(x / 2.);
        if (EWPT_mu.StatusFlag == Minimizer::MinimizerStatus::SUCCESS)
        {
          std::vector<double> checkmu;
          auto VEV_mu_sym = Minimizer::Minimize_gen_all(modelPointer,
                                                        EWPT_mu.Tc + 1,
                                                        checkmu,
                                                        startpoint,
                                                        args.WhichMinimizer);
          auto VEV_mu_brk = EWPT_mu.EWMinimum;
          auto eta_mu     = EtaInterface.CalcEta(args.vw,
                                             EWPT_mu.EWMinimum,
                                             VEV_mu_sym,
                                             EWPT_mu.Tc,
                                             modelPointer,
        ...
        {
          if (args.TerminalOutput)
            std::cout << "\tNo SFOEWPT found for given scale" << std::endl;
          continue;
        }
      } // END: Mu Factor
    }   // END:LineCounter
...
CLIOptions::CLIOptions(int argc, char *argv[])
{
  ...
  if (argc < 7 or args.at(0) == "--help")
  {
    ...
    std::cout << "EWBGRenormScale calculates the strength of the EWBG while "
                 "varying the MSBar renormalisation scale"
              << std::endl
              << "It is called either by " << std::endl
              << "./EWBGRenormScale Model Inputfile Outputfile  Line "
                 "NumberOfSteps Configfile"
              << std::endl
              ...
              << "The line in the input file with the parameter point. Expects "
              ...
              << "Number of Steps to vary the scale between 0.5 and 1.5 times "
                 "the original scale."
              << std::endl;

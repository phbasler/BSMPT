        ...
        linecounter != 1)
    {
      ...
      if (EWPTFlag == Minimizer::MinimizerStatus::SUCCESS)
      {
        if (C_PT * EWPT.Tc < EWPT.vc)
        {
          ...
          for (std::size_t i = 0; i < modelPointer->get_nVEV(); i++)
            basepoint.push_back(VEVSymmetric.at(i) +
                                0.5 * (vcritical.at(i) - VEVSymmetric.at(i)));

          auto SolMinPlane  = Minimizer::MinimizePlane(basepoint,
                                                      VEVSymmetric,
                                                      vcritical,
                                                      modelPointer,
                                                      EWPT.Tc,
          ...
          LW1D = Wall::calculate_wall_thickness_1D(
              modelPointer, EWPT.Tc, vcritical, VEVSymmetric);

          LWplane = Wall::calculate_wall_thickness_plane(modelPointer,
                                                         EWPT.Tc,
                                                         vcritical,
                                                         VEVSymmetric,
                                                         args.WhichMinimizer);
      ...
      if (args.FirstLine == args.LastLine)
      {
        ...
        if (EWPT.StatusFlag == Minimizer::MinimizerStatus::SUCCESS)
        {
          ...
          std::cout << "The potential barrier in the 1D approximation is given "
                       "by Vb = "
                    << Vb1D << " GeV^4" << std::endl;
          std::cout << "The wall thickness is then given as L_W = " << LW1D
                    << " GeV^-1 " << std::endl;
          std::cout << "The thick wall parameter is given by T_c L_W = "
                    << EWPT.Tc * LW1D << std::endl;

          std::cout << "Results of all dimensions : \n";
          std::cout << "The wall thickness is then given as L_W = " << LWplane
                    << " GeV^-1 " << sep << " = " << LWplane * 1.239842e-15
                    << " m " << std::endl;
          std::cout << "The thick wall parameter is given by T_c L_W = "
                    << EWPT.Tc * LWplane << std::endl;
        }
      }
    }
    linecounter++;
...
CLIOptions::CLIOptions(int argc, char *argv[])
{
  ...
  if (argc < 6 or args.at(0) == "--help")
  {
    ...
    std::cout << "WallThickness calculates the wall thickness" << std::endl
              ...
              << "./WallThickness Model Inputfile Outputfile  LineStart LineEnd"
              << std::endl

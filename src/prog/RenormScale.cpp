...
/**
 * @file
 * Calculates xi_C as a function of the renormalised scale mu. The
 * renormalisation scale mu is varied from 1/2 to 1.5 C_vev0 in NumberOfStep
 * steps.
 */
...
try
{
  ...
  while (getline(infile, linestr))
  {
    ...
    if (linecounter == 1)
    {
      ...
      outfile << linestr << sep << "mu_factor" << sep << "mu" << sep
              << modelPointer->addLegendTemp() << sep
              << modelPointer->addLegendVEV() << sep
              << modelPointer->addLegendCT() << sep << "BSMPT_StatusFlag"
              << std::endl;
    ...
    if (args.Line == linecounter)
    {
      ...
      auto EWPTdefault = Minimizer::PTFinder_gen_all(
      ...
      for (std::size_t i = 0; i < modelPointer->get_nVEV(); i++)
        startpoint.push_back(0.5 * EWPTdefault.EWMinimum.at(i));
      auto VEVsym = Minimizer::Minimize_gen_all(
          modelPointer, EWPTdefault.Tc + 1, checksym, startpoint);
      if (args.TerminalOutput)
        std::cout << "Start of mu variation" << std::endl;
      for (int step = 0; step < args.NumberOfSteps; step++)
      {
        double mu_factor =
            1 / 2. + (step / static_cast<double>(args.NumberOfSteps));
        auto VEVnames = modelPointer->addLegendTemp();
        auto CT_mu    = modelPointer->resetScale(C_vev0 * mu_factor);
        auto EWPT_mu  = Minimizer::PTFinder_gen_all(
            modelPointer, 0, 300, args.WhichMinimizer);
        std::vector<double> checkmu;
        auto VEV_NLO_mu = Minimizer::Minimize_gen_all(
            modelPointer, 0, checkmu, startpoint, args.WhichMinimizer);
        if (args.TerminalOutput)
        {
          std::cout << "\tStatusFlag = " << static_cast<int>(EWPT_mu.StatusFlag)
                    << " @ mu = " << modelPointer->get_scale() << " = "
                    << mu_factor << "*C_vev0" << std::endl;
          for (std::size_t i = 0; i < EWPT_mu.EWMinimum.size(); i++)
          {
            std::cout << sep << sep << VEVnames.at(i + 3) << sep
                      << EWPT_mu.EWMinimum.at(i) << " = "
        ...
        if (PrintErrorLines)
        {
          ...
          outfile << sep << EWPT_mu.Tc << sep << EWPT_mu.vc << sep
                  << EWPT_mu.vc / EWPT_mu.Tc << sep << EWPT_mu.EWMinimum;
...
CLIOptions::CLIOptions(int argc, char *argv[])
{
  ...
  if (argc < 6 or args.at(0) == "--help")
  {
    ...
    std::cout
        << std::boolalpha
        << "RenormScale varies the MSBar scale and calculates the EWPT at each "
           "scale between 0.5 and 1.5 times the set scale of the model"
        << std::endl
        << "It is called either by " << std::endl
        << "./RenormScale Model Inputfile Outputfile  Line NumberOfSteps "
           "TerminalOutput(y)"
        << std::endl
        ...
        << "The parameter point to investigate. Expects line 1 to be a legend."
        << std::endl
  ...
  if (UsePrefix)
  {
    for (const auto &arg : args)
    {
      ...
      {
        NumberOfSteps =
            std::stoi(el.substr(std::string("--numberofsteps=").size()));
      }

...
bool spline::make_monotonic()
{
  ...
  for (int i = 0; i < n - 1; i++)
  {
             ...
             (m_b[i] <= 0.0 && m_b[i + 1] <= 0.0 && avg < 0.0))
    {
      ...
      double r =
          std::sqrt(m_b[i] * m_b[i] + m_b[i + 1] * m_b[i + 1]) / std::fabs(avg);
      if (r > 3.0)
      {
...
void band_matrix::resize(int dim, int n_u, int n_l)
{
  ...
  for (std::size_t i = 0; i < m_upper.size(); i++)
  {
    m_upper[i].resize(dim);
  }
  for (std::size_t i = 0; i < m_lower.size(); i++)
  {
...
std::vector<double>
solve_quadratic(double a, double b, double c, int newton_iter)
{
  if (c == 0.0)
  {
...
std::vector<double>
solve_cubic(double a, double b, double c, double d, int newton_iter)
{
  if (d == 0.0)
  {
  ...
  else if (discr > 0)
  {
    ...
    z.resize(3);
    double ac = (1.0 / 3.0) * acos(q / (p * std::sqrt(p)));
    double sq = 2.0 * std::sqrt(p);
    z[0]      = sq * cos(ac);
  ...
  else if (discr < 0.0)
  {
    ...
    z.resize(1);
    double sgnq  = (q >= 0 ? 1 : -1);
    double basis = fabs(q) + std::sqrt(-discr);
    double C     = sgnq * std::pow(basis, 1.0 / 3.0); // c++11 has std::cbrt()
    z[0]         = C + p / C;
  }

...
{
  Model = ModelID::ModelIDs::CXSM; // global int constant which will be used to tell the program which model is called
 ...
 */
void Class_CxSM::ReadAndSet(const std::string& linestr, std::vector<double>& par )
{
//	std::cout << linestr << std::endl;
	...
	for(int k=1;k<=10;k++)
	{
	      ...
	      if(k==1) par[0] = tmp; // v_h
	      else if(k==2) par[1] = tmp; // v_s
	      else if(k==3) par[2] = tmp; // v_a
	      else if(k==4) par[3] = tmp; // m^2
	      else if(k==5) par[4] = tmp; // lambda
	      else if(k==6) par[5] = tmp; // delta_2
	      else if(k==7) par[6] = tmp; // b2
	      else if(k==8) par[7] = tmp; // d2
          else if(k==9) par[8] = tmp; // Reb1;
          else if(k==10) par[9] = tmp; // Imb1
          else if(k==11) par[10] = tmp; // Rea1;
          else if(k==12) par[11] = tmp; // Ima1
...
void Class_CxSM::set_gen(const std::vector<double>& par) {
    ...
    if(va!=0 and vs != 0){
        Rea1 = (-sqrt(0.2e1) * d2 * pow(va, 0.3e1) * vs - sqrt(0.2e1) * d2 * va * pow(vs, 0.3e1) - sqrt(0.2e1) * delta2 * va * vh * vh * vs + 0.4e1 * Ima1 * vs + Imb1 * sqrt(0.2e1) * va * va + Imb1 * sqrt(0.2e1) * vs * vs - 0.2e1 * sqrt(0.2e1) * b2 * va * vs) / va / 0.4e1;
        Reb1 = 0.1e1 / va * (d2 * pow(va, 0.3e1) + d2 * va * vs * vs + delta2 * va * vh * vh - 0.4e1 * Ima1 * sqrt(0.2e1) - 0.2e1 * Imb1 * vs + 0.2e1 * b2 * va) / 0.2e1;
        msq = -delta2 * va * va / 0.2e1 - delta2 * vs * vs / 0.2e1 - lambda * vh * vh / 0.2e1;
	}
    else if(va != 0 and vs == 0){
        Rea1 = sqrt(0.2e1) * Imb1 * va / 0.4e1;
        Reb1 = 0.1e1 / va * (d2 * pow(va, 0.3e1) + delta2 * va * vh * vh - 0.4e1 * Ima1 * sqrt(0.2e1) + 0.2e1 * b2 * va) / 0.2e1;
        msq = -delta2 * va * va / 0.2e1 - lambda * vh * vh / 0.2e1;
	}
    else if(va == 0 and vs != 0)
    {
        Ima1 = -sqrt(0.2e1) * Imb1 * vs / 0.4e1;
        Reb1 = -0.1e1 / vs * (d2 * pow(vs, 0.3e1) + delta2 * vh * vh * vs + 0.4e1 * Rea1 * sqrt(0.2e1) + 0.2e1 * b2 * vs) / 0.2e1;
...
//		if(testsum != 0) posG2 = i;
	...
	for(int j=0;j<3;j++) {
		for(int i=0;i<3;i++) NeutralMatrix(i,j) = HiggsRot(posN[i],j+3);
	}
...
std::vector<double> Class_CxSM::calc_CT() const
{
    ...
    {
        // You can either choose if Ima1 or Imb1 has a CT, in case Ima1 != 0 it will get the CT, otherwise Imb1
        parCT[0] = ((va * va + vs * vs) * HesseWeinberg(3, 5) - 3 * vh * va * (HesseWeinberg(2, 2) - HesseWeinberg(3, 3) / 3)) / vh / va;
        parCT[1] = (2 * HesseWeinberg(2, 2) - 2 * HesseWeinberg(3, 3)) * pow(vh, -2);
        parCT[2] = -2 / va / vh * HesseWeinberg(3, 5);
        parCT[3] = ((2 * va * va + 2 * vs * vs) * HesseWeinberg(4, 5) - (-HesseWeinberg(3, 5) * vh + va * (HesseWeinberg(4, 4) + HesseWeinberg(5, 5))) * vs) / vs / va;
        parCT[4] = -2 / vs / va * HesseWeinberg(4, 5);
        parCT[5] = HesseWeinberg(5, 5) + vs / va * HesseWeinberg(4, 5) - va / vs * HesseWeinberg(4, 5) - HesseWeinberg(4, 4);
        parCT[6] = 0;
        parCT[7] = (-HesseWeinberg(4, 5) * vs * vs + va * (vs * HesseWeinberg(4, 4) - NablaWeinberg(4))) * sqrt(2) / va / 2;
        parCT[8] = sqrt(2) * (va * va * HesseWeinberg(4, 5) - vs * (HesseWeinberg(5, 5) * va - NablaWeinberg(5))) / vs / 2;
    ...
    {
        parCT[0] = ((va * va + vs * vs) * HesseWeinberg(3, 5) - 3 * vh * (HesseWeinberg(2, 2) - HesseWeinberg(3, 3) / 3) * va) / vh / va;
        parCT[1] = (2 * HesseWeinberg(2, 2) - 2 * HesseWeinberg(3, 3)) * pow(vh, -2);
        parCT[2] = -2 / va / vh * HesseWeinberg(3, 5);
        parCT[3] = (2 * HesseWeinberg(4, 5) * vs + HesseWeinberg(5, 5) * va - HesseWeinberg(4, 4) * va + HesseWeinberg(3, 5) * vh - 2 * NablaWeinberg(5)) / va;
        parCT[4] = (-2 * HesseWeinberg(4, 5) * vs - 2 * HesseWeinberg(5, 5) * va + 2 * NablaWeinberg(5)) / (va * va + vs * vs) / va;
        parCT[5] = ((-pow(va, 3) - vs * vs * va) * HesseWeinberg(4, 4) + (-vs * va * va + pow(vs, 3)) * HesseWeinberg(4, 5) + 2 * HesseWeinberg(5, 5) * va * vs * vs + NablaWeinberg(5) * (va * va - vs * vs)) / (va * va + vs * vs) / va;
        parCT[6] = (2 * va * va * HesseWeinberg(4, 5) - 2 * vs * (HesseWeinberg(5, 5) * va - NablaWeinberg(5))) / (va * va + vs * vs);
        parCT[7] = (HesseWeinberg(4, 5) * (va * va - vs * vs) + HesseWeinberg(4, 4) * va * vs - HesseWeinberg(5, 5) * va * vs - NablaWeinberg(4) * va + NablaWeinberg(5) * vs) * sqrt(2) / va / 2;
        parCT[8] = 0;
        parCT[9] = -NablaWeinberg(0);
        parCT[10] = -NablaWeinberg(1);
        parCT[11] = -NablaWeinberg(2);
    ...
    {
        parCT[0] = (-3 * HesseWeinberg(2, 2) * vh + HesseWeinberg(3, 3) * vh + va * HesseWeinberg(3, 5)) / vh;
        parCT[1] = (2 * HesseWeinberg(2, 2) - 2 * HesseWeinberg(3, 3)) * pow(vh, -2);
        parCT[2] = -2 / va / vh * HesseWeinberg(3, 5);
        parCT[3] = (HesseWeinberg(3, 5) * vh - HesseWeinberg(4, 4) * va + HesseWeinberg(5, 5) * va - 2 * NablaWeinberg(5)) / va;
        parCT[4] = (-2 * HesseWeinberg(5, 5) * va + 2 * NablaWeinberg(5)) * pow(va, -3);
    ...
    {
        parCT[0] = (-3 * HesseWeinberg(2, 2) * vh + HesseWeinberg(3, 3) * vh + vs * HesseWeinberg(3, 4)) / vh;
        parCT[1] = (2 * HesseWeinberg(2, 2) - 2 * HesseWeinberg(3, 3)) * pow(vh, -2);
        parCT[2] = -2 / vh / vs * HesseWeinberg(3, 4);
        parCT[3] = (HesseWeinberg(3, 4) * vh + vs * HesseWeinberg(4, 4) - HesseWeinberg(5, 5) * vs - 2 * NablaWeinberg(4)) / vs;
        parCT[4] = (-2 * vs * HesseWeinberg(4, 4) + 2 * NablaWeinberg(4)) * pow(vs, -3);
...
void Class_CxSM::Debugging(const std::vector<double>& input, std::vector<double>& output) const
{
	...
	for(size_t i=0;i<NHiggs;i++){
	 ...
	 std::cout << "dT4 = " << HesseWeinberg(0,0) * vh - NablaWeinberg(3) << std::endl;
	 std::cout << "ahhsI = " << std::sqrt(2) * (HesseWeinberg(4,5) *va*va  - vs * (HesseWeinberg(5,5) *va - NablaWeinberg(5)))/(vh*vh*vs)
			 << std::endl;

	 std::cout << "HT = " << HesseWeinberg(3,5) *vs - HesseWeinberg(3,4)*va << std::endl
			 << "HT rel =  " << (HesseWeinberg(3,5) *vs - HesseWeinberg(3,4)*va)/(HesseWeinberg(3,5) *vs + HesseWeinberg(3,4)*va)
			 << std::endl;

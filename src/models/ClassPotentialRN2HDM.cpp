 ...
 */
std::string Class_Potential_RN2HDM::addLegendCT(){
  std::string out = "Du11sq\tDu22sq\tDm12sq\tDL1\tDL2\tDL3\tDL4\tDL5\tDussq\tDL6\tDL7\tDL8\tDT1\tDT2\tDTs";
  return out;
}
 ...
 */
std::string Class_Potential_RN2HDM::addLegendTripleCouplings(){
 ...
 * complement the legend of the given inputfile
 */
std::string Class_Potential_RN2HDM::addLegendVEV(){
	return "omega_{CB}\tomega_{CP}\tomega_1\tomega_2\tomega_s";
}



void Class_Potential_RN2HDM::ReadAndSet(const std::string& linestr, std::vector<double>& par )
{
	...
	for(int k=1;k<=12;k++)
	{
		...
		else if(k==10) lvs = tmp;
		else if(k==11) ltanbeta = tmp;
		else if(k==12) lu3 = tmp;
...
void Class_Potential_RN2HDM::set_gen(const std::vector<double>& p) {
  	...
  	u1 = RealMMix*TanBeta-0.5*v1*v1*L1-0.5*v2*v2*(L3+L4+RL5)-0.5*Nvs*Nvs*NL7;
  	u2 = RealMMix/TanBeta - 0.5*v2*v2*L2 - 0.5*v1*v1*(L3+L4+RL5) -0.5*Nvs*Nvs*NL8;
 ...
 */
void Class_Potential_RN2HDM::write() {
...
void Class_Potential_RN2HDM::calc_CT(std::vector<double>& par)
{
  bool Debug=false;
  	if(Debug) std::cout << "Start Debugging of " << __func__ << std::endl;
	...
	MatrixXd HesseWeinberg(NHiggs,NHiggs),HiggsRot(NHiggs,NHiggs);
  	...
  	if(Nvs != 0)
  	  {
  		VB0[0] = (double) ((Nvs * HesseWeinberg(6, 8) - HesseWeinberg(2, 3) * v2 + 2 * HesseWeinberg(4, 4) * v1 + HesseWeinberg(6, 7) * v2 - 5 * HesseWeinberg(2, 2) * v1 + HesseWeinberg(6, 6) * v1) / v1) / 0.2e1;
  	    	VB0[1] = (double) ((Nvs * HesseWeinberg(7, 8) * v2 + HesseWeinberg(7, 7) * v2 * v2 - 3 * HesseWeinberg(5, 5) * v2 * v2 - HesseWeinberg(2, 3) * v1 * v2 + 5 * v1 * v1 * HesseWeinberg(4, 4) + v1 * v2 * HesseWeinberg(6, 7) - 5 * HesseWeinberg(2, 2) * v1 * v1) * (double) pow((double) v2, (double) (-2))) / 0.2e1;
  	    	VB0[2] = (HesseWeinberg(2, 3) * v2 + HesseWeinberg(4, 4) * v1 - HesseWeinberg(2, 2) * v1) / v2;
  	    	VB0[3] = (-HesseWeinberg(4, 4) + 2 * HesseWeinberg(2, 2) - HesseWeinberg(6, 6)) * (double) pow((double) v1, (double) (-2));
  	    	VB0[4] = (-HesseWeinberg(7, 7) * v2 * v2 + HesseWeinberg(5, 5) * v2 * v2 - 2 * v1 * v1 * HesseWeinberg(4, 4) + 2 * HesseWeinberg(2, 2) * v1 * v1) * (double) pow((double) v2, (double) (-4));
  	    	VB0[5] = (HesseWeinberg(2, 2) * v1 + HesseWeinberg(2, 3) * v2 - HesseWeinberg(4, 4) * v1 - HesseWeinberg(6, 7) * v2) / v1 * (double) pow((double) v2, (double) (-2));
  	    	VB0[6] = 0;
  	    	VB0[7] = (2 * HesseWeinberg(4, 4) - 2 * HesseWeinberg(2, 2)) * (double) pow((double) v2, (double) (-2));
  	    	VB0[8] = (double) ((HesseWeinberg(8, 8) * Nvs + v2 * HesseWeinberg(7, 8) + v1 * HesseWeinberg(6, 8) - 3 * NablaWeinberg(8)) / Nvs) / 0.2e1;
  	    	VB0[9] = -(double) pow((double) Nvs, (double) (-3)) * (HesseWeinberg(8, 8) * Nvs - NablaWeinberg(8));
  	    	VB0[10] = -1 / v1 / Nvs * HesseWeinberg(6, 8);
  	    	VB0[11] = -1 / v2 / Nvs * HesseWeinberg(7, 8);
  	    	VB0[12] = HesseWeinberg(2, 2) * v1 + HesseWeinberg(2, 3) * v2 - NablaWeinberg(6);
  	    	VB0[13] = (HesseWeinberg(0, 0) * v1 * v1 + HesseWeinberg(2, 3) * v1 * v2 - v1 * v1 * HesseWeinberg(4, 4) + HesseWeinberg(5, 5) * v2 * v2 - NablaWeinberg(7) * v2) / v2;
  	    	VB0[14] = 0;
...
//  	else{
//  	  VB0[0] = (double) ((HesseWeinberg(2, 3) * v2 - 2 * HesseWeinberg(4, 5) * v2 + HesseWeinberg(6, 6) * v1 - 3 * HesseWeinberg(2, 2) * v1 + HesseWeinberg(6, 7) * v2) / v1) / 0.2e1;
//  	    	VB0[1] = (double) ((HesseWeinberg(2, 3) * v1 - 2 * HesseWeinberg(4, 5) * v1 - 3 * HesseWeinberg(3, 3) * v2 + HesseWeinberg(7, 7) * v2 + HesseWeinberg(6, 7) * v1) / v2) / 0.2e1;
//  	    	VB0[2] = 2 * HesseWeinberg(2, 3) - HesseWeinberg(4, 5);
//  	    	VB0[3] = -(HesseWeinberg(2, 3) * v2 - HesseWeinberg(4, 5) * v2 + HesseWeinberg(6, 6) * v1 - HesseWeinberg(2, 2) * v1) * (double) pow((double) v1, (double) (-3));
//  	    	VB0[4] = -(HesseWeinberg(2, 3) * v1 - HesseWeinberg(4, 5) * v1 - HesseWeinberg(3, 3) * v2 + HesseWeinberg(7, 7) * v2) * (double) pow((double) v2, (double) (-3));
//  	    	VB0[5] = (HesseWeinberg(4, 5) - HesseWeinberg(6, 7)) / v1 / v2;
//  	    	VB0[6] = 0;
//  	    	VB0[7] = 2 / v2 * (HesseWeinberg(2, 3) - HesseWeinberg(4, 5)) / v1;
//  	    	VB0[8] = -HesseWeinberg(8, 8);
//  	    	VB0[9] = 0;
//  	    	VB0[10] = 0;
//  	    	VB0[11] = 0;
//  	    	VB0[12] = HesseWeinberg(2, 3) * v2 + HesseWeinberg(2, 2) * v1 - NablaWeinberg(6);
//  	    	VB0[13] = HesseWeinberg(3, 3) * v2 + HesseWeinberg(2, 3) * v1 - NablaWeinberg(7);
//  	    	VB0[14] = 0;
//
//  	    	for(int i=0;i<15;i++)
//  	    	{
...
//  	}
  	...
  	Identities[2] = (HesseWeinberg(0, 0) * v1 * v1 + v2 * v2 * HesseWeinberg(5, 5) - v1 * v1 * HesseWeinberg(4, 4) - v2 * v2 * HesseWeinberg(3, 3));
  	Identities[3] = (HesseWeinberg(0, 0) * v1 - HesseWeinberg(4, 4) * v1 + HesseWeinberg(2, 3) * v2 - HesseWeinberg(4, 5) * v2);
  	Identities[4] = (HesseWeinberg(0, 0) * v1 * v1 + v2 * v2 * HesseWeinberg(5, 5) - v1 * v1 * HesseWeinberg(4, 4) - v2 * v2 * HesseWeinberg(1, 1));
...
void Class_Potential_RN2HDM::TripleHiggsCouplings()
{
  bool Debug=false;
	...
	for(int i=0;i<3;i++)
	{
		if(HiggsRot(i,0) != 0) posG1 = i;
		if(HiggsRot(i,2) != 0) posG2 = i;
		if(HiggsRot(i,5) != 0) posG0 = i;
	}
	for(int i=3;i<NHiggs;i++)
	{
		if(HiggsRot(i,0) != 0) posMHCS1 = i;
		if(HiggsRot(i,2) != 0) posMHCS2 = i;
		if(HiggsRot(i,6) != 0)
		{
			posN[countposN] = i;
			countposN++;
		}
		if(HiggsRot(i,5) != 0 ) posA = i;
	}
	...
	if(Debug) std::cout << "Higgs = " << HiggsRotSort << std::endl;
...
void Class_Potential_RN2HDM::MinimizeOrderVEV(const std::vector<double>& vevMinimizer, std::vector<double>& vevFunction){
...
//  return false;
  bool Debug = false;
  if(Debug) std::cout << "Debug turned on in Class_Potential_RN2HDM :: " << __func__ << std::endl;
  ...
  if(Debug)
    {
      std::cout << "Debye Diagonal :  \n ";
      for(int i=0;i<NHiggs;i++) std::cout << "D["<< i << "][" << i <<"] = " << DebyeHiggs[i][i] << std::endl;
    }
...
double Class_Potential_RN2HDM::VTreeSimplified(const std::vector<double>& v){
...
double Class_Potential_RN2HDM::VCounterSimplified(const std::vector<double>& v)
{
...
void Class_Potential_RN2HDM::Debugging(const std::vector<double>& input, std::vector<double>& output){

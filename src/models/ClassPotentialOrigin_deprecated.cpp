                                  ...
                                  int diff) const
{
  ...
  if (diff == 0)
  {
    for (int l = 0; l <= n; l++)
    {
      PotVal += 1 / (std::pow(2, l) * gsl_sf_fact(l)) * gsl_sf_gamma(2.5 + l) /
                gsl_sf_gamma(2.5 - l) * std::pow(Temp / Mass, l);
    }
    PotVal *= -std::exp(-Mass / Temp) *
              std::pow(Mass * Temp / (2 * M_PI), 1.5) * Temp;
  }
  else if (diff > 0)
  {
    for (int l = 0; l <= n; l++)
    {
      PotVal += 1 / (std::pow(2, l) * gsl_sf_fact(l)) * gsl_sf_gamma(2.5 + l) /
                gsl_sf_gamma(2.5 - l) * std::pow(Temp / Mass, l) *
                (2 * Temp * l + 2 * Mass - 3 * Temp);
    }
    PotVal *= 1.0 / (16 * M_PI) * (Temp / Mass) * std::exp(-Mass / Temp) *
              std::sqrt(2 * Mass * Temp / M_PI);
  }
  else if (diff == -1)
  {
    ...
    for (int l = 0; l <= n; l++)
    {
      Sum += 1 / (std::pow(2, l) * gsl_sf_fact(l)) * gsl_sf_gamma(2.5 + l) /
             gsl_sf_gamma(2.5 - l) * std::pow(Temp / Mass, l);
      SumDerivative += 1 / (std::pow(2, l) * gsl_sf_fact(l)) *
                       gsl_sf_gamma(2.5 + l) / gsl_sf_gamma(2.5 - l) *
                       std::pow(Temp / Mass, l) * l / Temp;
    }
    PotVal = std::pow(Temp, 2) * Sum * std::sqrt(MassSquared) / Temp +
             5 * Sum * std::pow(Temp, 2) / 2 - 2 * SumDerivative * MassSquared;
    PotVal *= -std::sqrt(2) * std::pow(MassSquared, 2) *
              std::exp(-std::sqrt(MassSquared) / Temp);
    PotVal *= 1.0 / (4 * std::pow(M_PI, 1.5) * std::sqrt(Temp) *
                     std::pow(MassSquared, 2.5));
  }
                                   ...
                                   int diff) const
{
  ...
  if (diff == 0)
  {
    ...
    PotVal += std::pow(MassSquared, 2) / (64 * M_PI * M_PI) *
              (logM - std::log(Temp * Temp) - cf);
    for (int l = 2; l <= n; l++)
    {
      PotVal -= MassSquared * Temp * Temp / 2.0 *
                std::pow(-MassSquared / (4 * M_PI * M_PI * Temp * Temp), l) *
                gsl_sf_doublefact(2 * l - 3) * gsl_sf_zeta(2 * l - 1) * 1 /
                (gsl_sf_doublefact(2 * l)) * 1.0 / (l + 1) *
                (std::pow(2, 2 * l - 1) - 1);
    }
  }
  else if (diff > 0)
  {
    ...
    PotVal += MassSquared * (logM - std::log(Temp * Temp) - cf) /
              (32.0 * M_PI * M_PI);
    PotVal += MassSquared / (64.0 * M_PI * M_PI);
    for (int l = 2; l <= n; l++)
    {
      double Kl = gsl_sf_doublefact(2 * l - 3) * gsl_sf_zeta(2 * l - 1) * 1 /
                  (gsl_sf_doublefact(2 * l)) * 1.0 / (l + 1) *
                  (std::pow(2, 2 * l - 1) - 1);
      PotVal += -0.5 * Temp * Temp *
                std::pow(-MassSquared / (4.0 * M_PI * M_PI * Temp * Temp), l) *
                Kl * (l + 1);
    }
  }
  else if (diff == -1)
  {
    ...
    for (int l = 2; l <= n; l++)
    {
      double Kl = gsl_sf_doublefact(2 * l - 3) * gsl_sf_zeta(2 * l - 1) * 1 /
                  (gsl_sf_doublefact(2 * l)) * 1.0 / (l + 1) *
                  (std::pow(2, 2 * l - 1) - 1);
      Sum += std::pow(-MassSquared / std::pow(Temp, 2) * 1.0 /
                          (4 * std::pow(M_PI, 2)),
                      l) *
             Kl;
      SumDerivative +=
          -2 / Temp *
          std::pow(-MassSquared / (4 * std::pow(M_PI * Temp, 2)), l) * Kl;
    }
                                   ...
                                   int diff) const
{
  ...
  double cb =
      1.5 + 2 * std::log(4 * M_PI) - 2 * ThermalFunctions::C_euler_gamma;
  if (diff == 0)
  {
    PotVal = -M_PI * M_PI / 90.0 * std::pow(Temp, 4);
    PotVal += MassSquared * Temp * Temp / (24.0);
    PotVal -= std::pow(MassSquared, 1.5) * Temp / (12.0 * M_PI);
    PotVal -= std::pow(MassSquared, 2.0) / (64 * M_PI * M_PI) *
              (LogTerm - std::log(Temp * Temp) - cb);
    for (int l = 2; l <= n; l++)
    {
      PotVal += MassSquared * Temp * Temp * 0.5 *
                std::pow(-MassSquared / (4 * M_PI * M_PI * Temp * Temp), l) *
                gsl_sf_doublefact(2 * l - 3) * gsl_sf_zeta(2 * l - 1) * 1 /
                (gsl_sf_doublefact(2 * l)) * 1.0 / (l + 1);
    }
  ...
  else if (diff > 0)
  {
    ...
    PotVal += -MassSquared * (LogTerm - std::log(Temp * Temp) - cb) /
              (32.0 * M_PI * M_PI);
    PotVal += -MassSquared / (64.0 * M_PI * M_PI);
    for (int l = 2; l <= n; l++)
    {
  ...
  else if (diff == -1)
  {
    ...
    for (int l = 2; l <= n; l++)
    {
      double Factorial = gsl_sf_doublefact(2 * l - 3) * gsl_sf_zeta(2 * l - 1) *
                         1 / (gsl_sf_doublefact(2 * l)) * 1.0 / (l + 1);
      Sum += std::pow(-MassSquared / (4 * std::pow(M_PI * Temp, 2)), l) *
             Factorial;
      SumDerivative +=
          -2 / Temp *
          std::pow(-MassSquared / (4 * std::pow(M_PI * Temp, 2)), l) *
          Factorial;
    }
    ...
    if (MassSquared > 0)
      PotVal += -std::pow(Temp, 3) * std::log(MassSquared / std::pow(Temp, 2)) /
                (16 * M_PI * M_PI);
    PotVal += std::pow(Temp, 3) * cb / (16 * M_PI * M_PI);
    if (MassSquared > 0)
      PotVal += std::pow(MassSquared, 2) / (12 * M_PI * std::sqrt(MassSquared));
    PotVal += std::pow(MassSquared, 2) * SumDerivative / Temp;
                                            ...
                                            int diff) const
{
  ...
  if (diff == 0)
  {
    ...
    else
    {
      ...
      {
        PotVal += Vsb(MassSquared, Temp, 3) + ThermalFunctions::C_BosonShift *
                                                  std::pow(Temp, 4) /
                                                  (2 * M_PI * M_PI);
      }
  ...
  {
    if (std::abs(MassSquared / std::pow(Temp, 2)) <= C_threshold)
                                              ...
                                              int diff) const
{
  ...
  if (diff == 0)
  {
    ...
    {
      PotVal = -(Vsf(MassSquared, Temp, 4) + ThermalFunctions::C_FermionShift *
                                                 std::pow(Temp, 4) /
                                                 (2 * M_PI * M_PI));
    }
  ...
  else
  {
    if (x < ThermalFunctions::C_FermionTheta)
    {
      ...
      if (diff == -1)
      {
        PotVal += -ThermalFunctions::C_FermionShift * 4 * std::pow(Temp, 3) /
                  (2 * M_PI * M_PI);
      }
    }
    else if (x >= ThermalFunctions::C_FermionTheta)

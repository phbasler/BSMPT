...
std::vector<double> TransformCoordinates(
	...
	vMin[params.Index] = 1.0/((params.normalvector.at(params.Index))) *(plane_const - plane_coord);
...
MinimizePlaneReturn MinimizePlane(const std::vector<double>& basepoint,
    ...
    if(IndexNorm == -1){
        throw std::runtime_error("The normalvector in the minimization of the plane is zero. \
...
int GSL_Minimize_Plane_From_S_gen_all(const struct PointerContainerMinPlane& params, std::vector<double>& sol,
                                ...
                                std::size_t MinSol)
{
	...
	if(numOfSol == 0)
	{
		std::cerr << "No solutions found during the GSL minimization at T = " << params.Temp << " GeV " << "\n";

...
/**
 * @brief This function check if the gradient increses from point_1 to point_2.
 ...
 */
bool MinimumTracer::GradientIncreases(
    const double &T,
...
{
  double gradOfPointM;
  double n                  = 10 * ceil(L2NormVector(point_2 - point_1) /
                       point_1.size()); // Number of points to be sampled.
  std::vector<double> delta = (1 / n) * (point_2 - point_1);

  double dV1 = L2NormVector(dV(point_1)) / delta.size();

  for (double m = 1.; m <= n; m++)
  {
    gradOfPointM = L2NormVector(dV(point_1 + m * delta)) / delta.size();
    if (gradOfPointM >= dV1 /*and gradOfPointM > 10 * GradientThreshold*/)
      return true;
  }
  stringstream ss;
  ss << "Gradient increases at T = " << T << ". Distance travelled "
     << L2NormVector(point_2 - point_1)
     << " | deltaV = " << V(point_1) - V(point_2) << " | VEV Step "
     << point_1 - point_2;
  BSMPT::Logger::Write(BSMPT::LoggingLevel::MinTracerDetailed, ss.str());
  return false;
}
...
{
  std::vector<std::vector<double>> res_vec;
  res_vec.push_back(min_false);
  auto diff_vec = min_true - min_false;
  if (npoints > 0)
  {
    auto step_vec = 1. / npoints * diff_vec;
    for (int i = 1; i <= npoints; i++)
    {
      res_vec.push_back(min_false + i * step_vec);
    }
  }
  return res_vec;
}
...
void Vacuum::PrintPhasesDiagram(int size)
{
  ...
  /*// Plotting smallest eigenvalue, used for debugging
  AsciiPlotter plotter("SEV for each phase", size, ceil(size / 3.));

  double eps = 0.1;
  int dim    = this->modelPointer->get_nVEV();
  std::function<double(std::vector<double>)> V_1;
  std::function<std::vector<double>(std::vector<double>)> dV_1;
  std::function<std::vector<std::vector<double>>(std::vector<double>)>
      Hessian_1;

  std::vector<std::vector<double>> plotT, plotSEV;
  ...
  for (std::size_t it = 0; it < PhasesList.size(); it++)
  {
    ...
    for (int i = 0; i < size; i++)
    {
      ...
      double SSEEVV = MinTracer->SmallestEigenvalue(temp, Hessian_1);

      std::cout << "Phase " << it << "had \t" << (SSEEVV - 1e-10)
                << " at T = " << TT << "\n";

      T.push_back(TT);
      SEV.push_back((SSEEVV - 1e-10)); // SSEEVV
    }
    plotT.push_back(T);
    plotSEV.push_back(SEV);
  }

  for (std::size_t it = 0; it < PhasesList.size(); it++)
  {

    plotter.addPlot(plotT[it],
                    plotSEV[it],
                    "SEV(Phase " + std::to_string(it) + ")",
                    markers[it % 6]);
  }

  plotter.legend();
  plotter.xlabel("T (GeV)");
  plotter.ylabel("SEV (GeV2)");
  plotter.show(ss);*/

  /* // Plotting the potential is hard, disabled for the moment
  AsciiPlotter plotter("V(T) for each phase", size, ceil(size / 3.));

  double lowestV  = 1e100;
  double HighestV = -1e100;
  char markers[]  = {".*oxcs"};
  std::vector<std::vector<double>> plotT, plotV;
  ...
  for (std::size_t it = 0; it < PhasesList.size(); it++)
  {
    ...
    for (int i = 0; i < size; i++)
    {
      double TT = PhasesList[it].T_low +
                  (double(i) / double(size - 1)) *
                      (PhasesList[it].T_high - PhasesList[it].T_low);
      double VV = PhasesList[it].Get(TT).potential;

      // ss <<"{ " << TT << "," << VV << "},\n";

      T.push_back(TT);
      V.push_back(VV / (1 + TT * TT));

      if (lowestV > VV) lowestV = VV;
      if (HighestV < VV) HighestV = VV;
    }
    plotT.push_back(T);
    plotV.push_back(V);
  }

  auto Normalizer = [&](double &pot)
  {
    pot = (pot - lowestV + 1);
    // pot = log(pot);
  };
  ...
  for (std::size_t it = 0; it < PhasesList.size(); it++)
  {
    ...
    plotter.addPlot(plotT[it],
                    plotV[it],
                    "V( Phase " + std::to_string(it) + ")",
                    markers[it % 6]);
  }

  plotter.legend();
  plotter.xlabel("T (GeV)");
  plotter.ylabel("V (GeV)");
  plotter.show(ss);*/

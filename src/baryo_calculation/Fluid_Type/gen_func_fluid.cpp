 ...
 */
        ...
        double Calc_Gam_M::hf(double w)
        {
            ...
            if (std::abs(res) < C_smallcut)
                res = 0;
            if (std::isnan(res))
                throw std::runtime_error("NaN @ Calc_Gam_M::hf");
            return res;
        }
        double Calc_Gam_M::hf_prime(double w)
        {
            ...
            if (std::abs(res) < C_smallcut)
                res = 0;
            if (std::isnan(res))
                throw std::runtime_error("NaN @ Calc_Gam_M::h_prime");
        ...
        void Calc_eta::operator()(const state_type &eta, state_type &deta, const double z)
        {
            ...
            deta[0] = Calc_eta::nL_cub(z) * std::exp(exponent_prefactor * z); //prefactor is not used here for numerical stability
        }
        ...
        void gen_fluid::top_func(double z, std::vector<double> &m_quark, std::vector<double> &m_quark_prime)
        {
            ...
            for (std::size_t i = 0; i < gen_nvev; i++)
                gen_phi.push_back(gen_fluid::gen_vcritical.at(i) / 2 * (1 - std::tanh(z / gen_fluid::LW)));
	  ...
	  */
        ...
        */
            ...
            if (std::isnan(res))
                throw std::runtime_error("NaN @ tau_func");
            m_lep.resize(1);

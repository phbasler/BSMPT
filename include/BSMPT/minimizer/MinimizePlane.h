...
/**
 * @brief The MinimizePlaneReturn struct contains the value of the potential at the minimum and the minimum
 */
...
/**
 * Struct which will store all necessary information to translate between the nVEV coordinates of the potential
 * and the nVEV -1 coordinates of the plane.
 */
 ...
 * to the connection vector is then calculated and the potential is minimised along this plane.
 * @param basepoint Parameter point at which the plane and the connection between the symmetric and
 * the broken minimum should be calculated
 ...
 * @return MinimizePlaneReturn struct which has the minimum and the value of the potential
 */
 ...
 * @param VEVSymmetric Symmetric minimum
 * @param VEVBroken Broken minimum
 * @param modelPointer Pointer to the corresponding model and parameter point through Class_Potential_Origin
 * @param Temp Temperature at which the minimum should be calculated
 * @return MinimizePlaneReturn struct which has the minimum and the value of the potential
 */
MinimizePlaneReturn MinimizePlane(const std::vector<double>& basepoint,
 ...
 * @param vMinTilde A point in the nVEV -1 dimensions which were used to minimise the potential in the plane
 * @param params the PointerContainerMinPlane struct which containts the potential and the index at which the vector has to
 * be extended.
 * @return The point vMinTilde will be extended by the missing component to be in the nVEV dimensional space which can
 * be used to calculate the potential
 */
std::vector<double> TransformCoordinates(
        const std::vector<double>& vMinTilde,
 ...
 * @param p Pointer to a PointerContainerMinPlane struct which contains the model information
 */
 ...
 * @param p GSL_params struct which containts the information about the model and the potential
...
/**
 * Minimise the Potential from different random starting points and choose the local minimum with the
 * deepest potential value as the candidate for the global minimum
 * @param params PointerContainerMinPlane which containts the model information and potential

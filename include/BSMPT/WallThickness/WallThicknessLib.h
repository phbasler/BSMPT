...
/**
 * @brief Calculate the Wall thickness using the minimization along the normal plane
...
double calculate_wall_thickness_1D(
        ...
        const std::vector<double>& vevsymmetric);
/**
 * @brief GSL_Find_Maximum_line finds the maximum of the function along a line using GSL for the maximisation
 * @param modelPointer Pointer to parameter point
 * @param Temp temperature at which the wall thickness is to be calculated
 * @param vcritical the electroweak VEV in the broken minimum
 * @param vevsymmetric the electroweak VEV in the symmetric minimum
...
/**
 * @brief GSL_Maximize_From_S_gen_line finds the next local maximum from the initial guess
 * @param params params struct containing the parameters needed for the maximisation
 * @param solution List of solutions which will be expanded with the new solution
 ...
 */
double GSL_VEFF_gen_all_maximum_line(double t, void *p); // 1D Maximization along the line from 0 to omega

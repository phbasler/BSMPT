...
/**
 * This class handles the evaluation of the transport equations as shown in
 * arXiv:hep-ph/0605242v2 Eq (44) and (45) with the thermal velocity. The
 * transport equations with second order ODEs in the chemical potentials is
 * given in
 * TODO:: paper/thesis
 */
...
protected:
  /**
   * @brief UseTanBetaSuppression Use the thermal tanbeta suppression in the
   * calculation of the theta(z)
   */
  bool UseTanBetaSuppression = false;
  /**
   * If true the transport equations with the plasma velocitys are used,
   * otherwise the second order transport equations in the chemical potentials
   * are used.
   */
  ...
  /**
   * Pointer to the specific model under investigation. At this moment the
   * transport equations are only implemented and tested for the C2HDM.
   */
  std::shared_ptr<Class_Potential_Origin> modelPointer;

  /**
   * Internal storage for the value of the VEVs at the broken minimum at the
...
public:
   ...
   * @param x Depending on transport_equations::UseVelocityTransportEquations
   * the value of the chemical potentials and either their derivatives or the
   * thermal velocities at the point z.
   * @param dxdt Here the derivative of x at the point z will be stored
   * @param z The distance to the wall (z = + infinity is the symmetric minimum,
   * z = - infinity is the broken minimum and z=0 is the wall)
   */
  transport_equations &
  operator()(const state_type &x, state_type &dxdt, const double /* z */);

  /**
* Calculates the top mass and its derivative w.r.t the single components of the
VEVs.
* @param vev The VEV configuration at which the mass and the derivatives should
be calculated.
*
* @return vector which will store the result. Will be cleared in the process of
the function call. This will have 9 entries, the first is the mass and then the
8 derivatives.
*/
  std::vector<double>
  get_top_mass_and_derivative(const std::vector<double> &vev) const;

  /**
   * Calculates the W mass at a given VEV and temerpature. As we assume no
   * charge breaking vev in the c2hdm we look for the degenerate eigenvalue in
   ...
   * @param z Distance to the wall. z < 0 is inside the broken phase and z > 0
 ...
 * @param z Distance from the wall (z=0) at which the equations should be
 * solved. The symmetric phase is at z=infinity.
 * @param parStart The boundary conditions in the symmetric phase for the
 * chemical potentials
 * @param params The GSL_integration_mubl struct which contains all necessary
 ...
 * @param p GSL_integration_mubl object which containts the necessary
 * information about the integration
 * @param nstep number of steps between the wall (z=0) and the symmetric phase
 * (z=3 LW )
 * @return GSL_mubl_interpolation object in which the created spline will be
 * stored
 */
...
/**
 * evaluates the spline and multiples it with the prefacotr exp(-nu z) in
 * 0605242 Eq (48)
 * @param z distance from the wall

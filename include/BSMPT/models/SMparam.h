...
/* Here is an example of the CKM Matrix given by the standard parameters. The elements V11, V23 and V33 are real in this parametrisation
 * and are calculated by the other elements and the unitarity conditions. If the unitarity at numerical precision is not
 * given you will end up with massive charged Goldstone bosons
*/

/**
 * @brief The lambda parameter in the Wolfenstein parametrisation of the CKM-Matrix
...
/**
 * @brief The theta_12 mixing angle in the CKM matrix calculated by the Wolfenstein parameters
 */
const double theta12 = std::asin(C_Wolfenstein_lambda);
/**
 * @brief The theta_23 mixing angle in the CKM matrix calculated by the Wolfenstein parameters
 */
const double theta23 = std::asin(C_Wolfenstein_A*std::pow(C_Wolfenstein_lambda,2));
/**
 * @brief The CP-violating angle in the CKM matrix calculated by the Wolfenstein parameters
 */
const double delta = std::arg(C_Wolfenstein_A*std::pow(C_Wolfenstein_lambda,3)*(C_Wolfenstein_rho+II*C_Wolfenstein_eta));
/**
 * @brief The theta_13 mixing angle in the CKM matrix calculated by the Wolfenstein parameters
 */
const double theta13 = std::asin(std::abs(C_Wolfenstein_A*std::pow(C_Wolfenstein_lambda,3)*(C_Wolfenstein_rho+II*C_Wolfenstein_eta)));
...
const std::complex<double> C_Vcd = -std::sin(theta12)*std::cos(theta23)
        - std::cos(theta12)*std::sin(theta23)*std::sin(theta13)*std::exp(II*delta);
/**
 * @brief cs element of the CKM matrix
 */
const std::complex<double> C_Vcs = std::cos(theta12)*std::cos(theta23)
        -std::sin(theta12)*std::sin(theta23)*std::sin(theta13)*std::exp(II*delta);
/**
...
const std::complex<double> C_Vtd = std::sin(theta12)*std::sin(theta23)
        -  std::cos(theta12) * std::cos(theta23)*std::sin(theta13)*std::exp(II*delta);
/**
 * @brief ts element of the CKM matrix
 */
const std::complex<double> C_Vts = -std::cos(theta12)*std::sin(theta23)-std::sin(theta12)*std::cos(theta23)*std::sin(theta13)*std::exp(II*delta);
/**
 ...
 */
const double C_sinsquaredWeinberg = 1-(C_MassW*C_MassW)/(C_MassZ*C_MassZ);
...
/**
 * @brief gauge coupling of the U(2)_L with the SM Higgs doublett, derived through the W-Boson mass and the SM VEV
 * Unit: GeV
 */
const double C_g = 2*C_MassW/C_vev0;
/**
 * @brief gauge coupling of the U(1) with the SM Higgs doublett, derived through the W- and Z-Boson masses and the SM VEV
 */
const double C_gs = 2*std::sqrt(std::pow(C_MassZ,2)-std::pow(C_MassW,2))/C_vev0;

/**
 * @brief Trilinear coupling between three SM Higgs Boson, calculated as the third derivative of the SM Higgs Potential
 * Unit: GeV
 */

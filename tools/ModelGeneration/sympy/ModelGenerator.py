...
class ModelGenerator:
    ...
    def printSimplifiedHiggsMassMatrix(self):
        print("Eigen::MatrixXd HiggsMass = Eigen::MatrixXd::Zero(NHiggs,NHiggs);")
        for i in range(self._nHiggs):
            vi = self._VEVAtFiniteTemp[i][1]
            if vi != 0:
                print("double " + self.convertToCPP(vi) + " = v.at(" + str(i) + ");")
        ...
        for i in range(self._nHiggs):
            for j in range(self._nHiggs):
                ...
                for k in range(self._nHiggs):
                    ...
                    tmp = tmp.subs(fieldsZero) * vk
                    val += simplify(tmp)
                    for l in range(self._nHiggs):
                        vl = self._VEVAtFiniteTemp[l][1]
                        tmp = diff(
                            self._VHiggs,
                            self._HiggsFields[i],
                            self._HiggsFields[j],
                        ...
                        tmp = tmp.subs(fieldsZero) * 0.5 * vk * vl
                        val += simplify(tmp)
                if val != 0:
                    print(
                        "HiggsMass("
